...
static void mavlink_test_aaa_ocean_listener(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_AAA_OCEAN_LISTENER >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_aaa_ocean_listener_t packet_in = {
        93372036854775807ULL,17651,17755
    };
    mavlink_aaa_ocean_listener_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.time_usec = packet_in.time_usec;
        packet1.air_conditioner = packet_in.air_conditioner;
        packet1.boat_speed = packet_in.boat_speed;
        
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_AAA_OCEAN_LISTENER_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_AAA_OCEAN_LISTENER_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aaa_ocean_listener_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_aaa_ocean_listener_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aaa_ocean_listener_pack(system_id, component_id, &msg , packet1.air_conditioner , packet1.boat_speed , packet1.time_usec );
    mavlink_msg_aaa_ocean_listener_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aaa_ocean_listener_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.air_conditioner , packet1.boat_speed , packet1.time_usec );
    mavlink_msg_aaa_ocean_listener_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_aaa_ocean_listener_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aaa_ocean_listener_send(MAVLINK_COMM_1 , packet1.air_conditioner , packet1.boat_speed , packet1.time_usec );
    mavlink_msg_aaa_ocean_listener_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}

static void mavlink_test_aaa_offboard_to_fcu(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_AAA_OFFBOARD_TO_FCU >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_aaa_offboard_to_fcu_t packet_in = {
        17235
    };
    mavlink_aaa_offboard_to_fcu_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.power_on = packet_in.power_on;
        
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_AAA_OFFBOARD_TO_FCU_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_AAA_OFFBOARD_TO_FCU_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aaa_offboard_to_fcu_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_aaa_offboard_to_fcu_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aaa_offboard_to_fcu_pack(system_id, component_id, &msg , packet1.power_on );
    mavlink_msg_aaa_offboard_to_fcu_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aaa_offboard_to_fcu_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.power_on );
    mavlink_msg_aaa_offboard_to_fcu_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_aaa_offboard_to_fcu_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aaa_offboard_to_fcu_send(MAVLINK_COMM_1 , packet1.power_on );
    mavlink_msg_aaa_offboard_to_fcu_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_aaa_ocean_listener(system_id, component_id, last_msg);
    mavlink_test_aaa_offboard_to_fcu(system_id, component_id, last_msg);
